...
data class WslDistribution(val wslExecutablePath: Path, val guid: String, val distributionName: String, val startArguments: List<String>, val defaultHomeDirectory: String?)
...
object Wsl {
    ...
    private fun isWslDashDashCdAvailableForLinuxPaths(): Boolean {
        return (osVersion?.toIntOrNull() ?: 0) >= 19041
    }
    ...
    fun enumerateDistributions(): List<WslDistribution> {
        ...
        return distroGuids.mapNotNull { distroGuid ->
            ...
            val distroName = runCatching {
                println("Get-ItemPropertyValue -Path HKCU:\\$RegKeyLxss\\$distroGuid -Name $RegValueDistributionName")
            ...
            WslDistribution(wslExePath, distroGuid, distroName, listOf("-d", distroName), defaultWslHome)
        }
    }
